...
func configCommand(apiClient *pesakit.Client) *clix.Command {
	flags := []clix.Flag{
		&clix.BoolFlag{
			Name:    "tigo",
			Aliases: []string{"t"},
		...
		&clix.BoolFlag{
			Name:    "file",
			Aliases: []string{"f"},
			Usage:   "upload config from .env file",
		},
	}
	return &clix.Command{
		Name:        "config",
		Usage:       "print pesakit configs",
		Description: "print pesakit configurations of mnos",
		Flags:       flags,
		Action:      printConfig(apiClient),
	}
}

func printConfig(apiClient *pesakit.Client) clix.ActionFunc {
	return func(ctx *clix.Context) error {
		...
		mc := apiClient.Mpesa.Conf
		tc := apiClient.TigoPesa.Config
		ac := apiClient.AirtelMoney.Conf
